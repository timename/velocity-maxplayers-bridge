...
- MMMFlight 1.11.0 为可选集成；安装后桥接会注册其公开 Presence 服务，未安装时统计桥接仍可用。
...
当 MMMFlight 1.11.0 存在时，Purpur 端通过 Bukkit `ServicesManager` 注册其 `local.mmm.flight.api.ProxyPresenceService`。`lookup(UUID)` 立即完成 `CompletableFuture`；缓存未命中时异步发起一次去重请求，不会同步等待网络。状态包括 `ONLINE`、`OFFLINE_RECENT`、`OFFLINE_LONG` 和 `UNKNOWN`。缓存有效期为 30 秒，离线最近一小时返回 `OFFLINE_RECENT`。
